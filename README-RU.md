...
Chaimu синхронизирует внешнее аудио с видео.
...
Этот код связывает внешнюю аудиодорожку с видео. Воспроизведение, перемотка и изменение скорости синхронизируются с видеоэлементом.

Если браузер поддерживает Web Audio API, Chaimu выбирает `ChaimuPlayer`. В остальных случаях используется `AudioPlayer`. Чтобы всегда использовать `AudioPlayer`, укажите `preferAudio`:
...
Начальный `url` указывать необязательно. Chaimu можно инициализировать без аудиодорожки и добавить её позже:
...
При смене видеоэлемента используйте `replaceVideo()`. Текущий плеер, аудиодорожка, громкость и `AudioContext` при этом сохранятся:

```js
await chaimu.replaceVideo(nextVideoEl);
```

Когда Chaimu больше не нужен, вызовите `destroy()`. После этого экземпляр нельзя использовать повторно:
...
Чтобы запустить демо из текущего репозитория:
